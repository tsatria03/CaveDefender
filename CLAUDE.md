...
It is a **client/server** game: connect to the server, log into an account, chat with other players, and walk around a shared map. Map objects are spawned through a custom game-engine DLL. The chat-center milestone is complete (accounts, public/private/local/staff/team chat, slash commands, networked player positions with locator beacons, live spatial voice chat). Gameplay is **Wallbreaker** (see [[game-vision-wallbreaker]]) in three modes: **PVE** (defend four walls from enemy bots), **EVP** (attack the walls from outside while builder bots defend — [[game-mode-evp]]), and **PVP** (humans on both sides, one team attacking, one defending — [[game-mode-pvp]], built and feature-complete).
