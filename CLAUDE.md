...
- `globals/player.nvgt` — the client-side roster (`players`, an array of the `player` class: `name`, `x`, `y`, a `beacontimer` and `beaconsound` handle) of **other** players the server reports, for placing their sounds in 3D. The local player stays `me` + identity globals. New; the class/helpers (`add_player` / `set_player_position` / `remove_player` / `get_player_index`) exist but aren't populated or wired yet.
