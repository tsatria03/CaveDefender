...
- `globals/dec.nvgt` — central client state: player identity (`name`, `password`, `gender`), prefs, sound pools, timers, and the `st` savedata. The cached last account (name/password/gender) is stored inside the same `settings.cvf` as prefs, not a separate file. (The version string now lives in `client/includes/version.nvgt`, not here.)
...
The version string lives in **two** files that must stay in sync: `client/includes/version.nvgt` (client) and `server/includes/version.nvgt` (server), each just `const string version="x.y";` (included from each side's `includes.nvgt` before the glob-includes). Bump **both** together when opening a new changelog version block.
