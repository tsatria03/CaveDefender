...
Server-side. Each account is a key=value file at `server/data/players/<username>.usr`: `name`, `salt`, the **SHA-256 hash of salt+password**, `gender`, `rank`. `rank` is one of `player`, `moderator`, `administrator`, or `developer` (mutually exclusive, **full words only** — no short forms). `rank_level()` maps them to `0/1/2/3` and **commands are gated by minimum level** (`/kick` ≥ moderator, `/motd`/`/restart`/`/fastrestart`/`/exit` ≥ administrator, `/promote`/`/demote` ≥ administrator). `is_staff` (= level ≥ 1) is for **display only** (chat-name prefix, `/staff` listing, the "you are now…" notice). `/promote`/`/demote` enforce a **grant cap** (you can only set a rank ≤ your own level) and **strictly-below target protection** (you can't act on someone at or above your level — which also blocks self-promote/demote). Helpers live in `server/includes/main/globals/account.nvgt` (`account_exists`, `create_account`, `load_account`, `verify_password`, `get_rank`, `set_rank`, `rank_level`, `is_staff`). Passwords are **never** stored in plaintext. The on-disk rank is loaded into the in-memory `player` roster at spawn; there is no longer a `dev` backdoor (uncompiled builds used to auto-elevate — removed).
...
- `includes/main/globals/net.nvgt` — send/receive, message dispatch, `login` / `register_account`, chat broadcast, the word `filter`, and the staff/moderation commands gated by `rank_level` (`/kick`, `/motd`, `/exit`, `/restart`, `/fastrestart`, `/promote`, `/demote`; plus the public `/pm`, `/who`, `/staff`).
