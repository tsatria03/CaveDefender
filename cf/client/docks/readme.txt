...
If you are staff, the card also shows action buttons for the things your rank lets you do to that player: kick, ban, promote, demote, notify, warn, silence, set their nickname, reset their password, and delete their account. Only the buttons you are actually allowed to use on that particular player appear, and choosing one asks for anything it needs, such as a ban length, a new rank, a notification, a warning level and reason, the channel to silence and how long, a new nickname, a new password, or a warning to confirm a deletion, before carrying out the same action as the matching staff command. Resetting a password and deleting an account are administrator actions that work whether the player is online or offline, so you can use them from an offline player's card too. Silencing a player is the staff mute that stops everyone hearing them in a channel you choose, which is different from the personal mute above that quiets them only for you; among the channels you can choose is their online and offline sounds, which makes their connect and disconnect standard for everyone, and the same button also lifts a silence you have set. They never let you do anything the commands would not, so all the usual rank rules still apply; the staff help explains each of these in full.

The player list.

The F6 players menu shows only who is online right now. To see everyone who has an account, including players who are offline, type /playerlist, or /prl. Add on, or online, to list only the players currently online, or off, or offline, to list only those who are not; with nothing after it, a short menu lets you choose which. The online list is alphabetical, each name marked as online, while the offline list puts the most recently seen first and shows when each was last online, for example, arron (offline) Last seen, July 4, 2026 at 1:32 PM. Choosing anyone opens the same information card the players menu opens. For an online player it works exactly as above; for an offline one the card leaves off what only applies while they are connected, such as their location and the kick, message, and notify buttons, and keeps the actions that still make sense, such as ban, warn, silence, and the personal mutes.
