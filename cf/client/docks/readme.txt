...
The F6 players menu shows only who is online right now. To see everyone who has an account, including players who are offline, type /playerlist, or /prl. Add on, or online, to list only the players currently online, or off, or offline, to list only those who are not; with nothing after it, a short menu lets you choose which. Both lists are alphabetical, and offline names show when each was last online, for example, arron (last seen, July 4, 2026 at 1:32 PM); to check one player's status on its own, use /seen. Choosing anyone opens the same information card the players menu opens. For an online player it works exactly as above; for an offline one the card leaves off only what needs the player present, the kick, private message, and notify buttons, and keeps everything else, ban, warn, silence, set nickname, promote, demote, reset password, delete account, and the personal mutes.
