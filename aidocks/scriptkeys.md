...
**Feature (dev, 2026-08, 5.0):** client-side "script keys" — user-defined keyboard shortcuts that run a slash command (or plain chat) from anywhere. Config file `cf/client/data/scriptkeys.srk` (dev moved it here from `cf/client/scriptkeys.txt` and renamed the extension, 2026-08), one command per line, line N → key N by position.
...
**Live update (dev-locked):** NO persistent in-memory copy — RE-READ `data/scriptkeys.srk` fresh on every script-key press (the `file` object; presses are user-initiated + the file is tiny, so cost is nil), so a hand-edit is live on the next press with no restart/reload/watcher. This means `data/scriptkeys.srk` MUST stay a LOOSE editable file in the data/ folder next to the game and be EXCLUDED from the Enigma box AND any pack (dev-confirmed) — otherwise players can't edit it.

**Boxing/build — DONE & verified:** the box excludes it structurally — `src/client/cfc.evb` never references it, and `scripts/gen_cfc_evb.py` only embeds `EMBED_FOLDERS = ["sounds","docks"]` + fixed lib DLLs, so a `cf/client/data/` file is never boxed (the post-box strip only removes embedded sounds/docks/DLLs). Build ships it via the client's `data/` folder: `CLIENT_ASSETS = ["lib","sounds","docks","data"]` copies `cf/client/data/` (holding `scriptkeys.srk`) into the bundle, and since `data` is NOT in `CLIENT_EMBEDDED_FOLDERS`, it stays a loose, editable, unboxed folder next to the exe. (History: originally a bundle-ROOT file shipped via a `loose_files` param + `CLIENT_LOOSE = ["scriptkeys.txt"]`; when the dev moved it into `data/` and renamed it `.srk` in 2026-08, the build switched to the folder copy and `CLIENT_LOOSE` was emptied.) See [[enigma-boxing]], [[path-conventions]].

**Loader/executor/bindings — BUILT (client, `includes/main/globals/scriptkeys.nvgt`):**
- `read_scriptkeys()` — reads `data/scriptkeys.srk` fresh each call (missing/unreadable → empty array), reusing `split_lines()` (extrafuncts) which CRLF-normalizes and keeps blank lines so line N stays mapped to key N. Path is cwd-relative `"data/scriptkeys.srk"` (cwd = `cf/client/` in dev, bundle root in release).
...
- Docs: changelog entry + readme "Script keys." section (NOT the player help — help is slash-commands-only). Todo Finished. Sample `cf/client/data/scriptkeys.srk` shipped loose. Build copies the client `data/` folder via `CLIENT_ASSETS` (see above).
