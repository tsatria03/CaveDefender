...
**Client code:**
- `net.nvgt`: globals `sound_cache` (dict relpath→cache file path, per session) + `sound_cachefile()` (slashes/spaces→underscores under `DIRECTORY_APPDATA + "tsatria03/CaveDefender/soundcache/"`); handlers `soundplay` (cache hit → `p.play_stationary`, miss → `getsound`) and `soundpush` (decode → cache file → set dict → play).
