...
Confirmed via this source: `bool natural_number_sort(const string&in, const string&in)` is a registered global (`src/misc_functions.cpp`) and `array<T>` has `void sort(const less&in, uint startAt=0, uint count=uint(-1))` (`ASAddon/src/scriptarray.cpp`), whose `less` funcdef is `bool(const T&in, const T&in)` — so `myStringArray.sort(natural_number_sort)` gives natural-number ordering (misc2 before misc10) with no hand-rolled comparator (used by the /playsound picker, see [[soundboard]]). **GOTCHA (bit us 2026-08, runtime "Index out of bounds" crash):** `CScriptArray::Sort` throws when `startAt >= numElements`, and for an EMPTY array (default startAt=0) that's `0 >= 0` → it THROWS on an empty array instead of no-opping. ALWAYS guard `if(arr.length() > 1) arr.sort(...)` — `find_files()` returns empty for a folder that holds only sub-folders, which is exactly how the soundboard picker hit it.
...
**LOCAL ENGINE MODIFICATION (2026-08):** `src/UI.cpp` `simple_file_dialog` (shared by open/save/folder dialogs) had `while (!fdi.tryWait(50)) SDL_PumpEvents();` -- a 50 ms Poco wait that pumped SDL events only ~20 Hz, starving the native picker's message loop (sluggish to open + navigate, which the onoff self-upload form exposed). Changed 50 -> 1 (~1000 Hz; 5 was still a bit slow). Engine must be REBUILT (nvgt.exe) for it to take effect; the dev rebuilds/tests. This means the pinned nvgt.exe now differs from stock Legacy-NVGT after a rebuild.
