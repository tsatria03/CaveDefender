...
**Item 3 is being built in two sections. 3a (spawn + reinforce) BUILT in 4.6:** added "Add cavern builders" + "Clear cavern builders" to `push_cavern_menu`; `do_cavern_action` handles `addbuilders` (ground-wood gate) and `clearbuilders` (host clear via `clear_builders`); new `cavernbuildercount` handler (`net.nvgt`) spawns via `spawn_builders`, cap 100, ground-wood re-check; and `builder_tick` (`builder.nvgt`) mode-branched in 3 spots so free builders work — phase gate now allows free's "active", reinforce cap = `wall_overbuild_cap` (400%) for free, and the EVP damage-floor ratchet is guarded `mode=="evp"` so free skips it. EVP path untouched. Still HOST-ONLY (guest access is item 6). **3b STILL TODO (see below): B locates builders in free + tally relocation.** CHANGELOG for item 3 is deferred until 3b lands (one entry for the whole builder feature).

Original spec (3b portion still pending): Reuse `spawn_builders(gameid, count)` (`builder.nvgt:40`) — builders roam and auto-target the weakest standing wall, so NO wall picker; the menu just asks **"how many builder bots?"**.
